...
if($result->num_rows > 0) { 
 ...
 while($row = $result->fetch_array()) {
 	...
 	// active 
 	if($row[4] == 1) { 		
 		$paymentStatus = "<label class='label label-success'>Pagado Completamente</label>";
 	} else if($row[4] == 2) { 		
 		$paymentStatus = "<label class='label label-info'>Avance de Pago</label>";
 	} else { 		
	...
	<div class="btn-group">
	  ...
	  <ul class="dropdown-menu">
	    ...
	    <li><a type="button" data-toggle="modal" id="paymentOrderModalBtn" data-target="#paymentOrderModal" onclick="paymentOrder('.$orderId.')"> <i class="glyphicon glyphicon-save"></i> Registrar Pago</a></li>

	    <li><a type="button" onclick="printOrder('.$orderId.')"> <i class="glyphicon glyphicon-print"></i> Imprimir Orden </a></li>
